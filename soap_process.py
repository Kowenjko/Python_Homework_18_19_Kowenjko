...
async def get_items_from_section(section_id):
    section_list = await get_response()
...
def start_parsing(id):
    return asyncio.run(get_items_from_section(id))
...
def main():
    sec_list = asyncio.run(get_response())

    ukr_data = {}
    NUM_SECTION = len(sec_list)
    NUM_CORES = cpu_count()
    ...
    with C_F.ProcessPoolExecutor(NUM_CORES) as executor:
        ...
        futures = [executor.submit(start_parsing, i)
...
if __name__ == "__main__":
    ...
    start = time.time()
    main()
    print_txt('Parallelism - ProcessPoolExecutor', start)
